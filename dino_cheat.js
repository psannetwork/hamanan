...
// iキーが押されたときに実行する関数
function onIKeyPressed() {  // 走行距離を計算して更
  const distanceRan = parseFloat(score) / Runner.instance_.distanceMeter.config.COEFFICIENT;  Runner.instance_.distanceRan = distanceRan;  // ジャンプ速度を設定
  Runner.instance_.tRex.setJumpVelocity(parseFloat(jumpVelocity));  // ゲームの速度を設定
  Runner.instance_.setSpeed(speedValue);  // 重力を設定
  Runner.instance_.tRex.config.GRAVITY = gravityValue;}// キー入力を監視してiキーが押されたら関数を実行
document.addEventListener("keydown", function(event) {  if (event.keyCode === 73) {    onIKeyPressed();  }});
